...
import {
  ...
  CATEGORY_SUBCATEGORY_TAB_NAME,
  CATEGORY_SUB_CATEGORY_MASTER_DATA,
  PAYMENT_METHODS_MASTER_DATA,
  PAYMENT_METHOD_TAB_HEADERS,
  PAYMENT_METHOD_TAB_NAME,
  TRANSACTION_COLUMNS_ORDERS,
...
export const getTransactionsData = async (): Promise<GetSheetData> => {
  ...
  try {
    const { data } = await instance.get(
      `/${getItem(LOCAL_SESSION_KEYS.SHEET_ID)}/values/${TRANSACTION_TAB_NAME}!A1:Z1000?majorDimension=ROWS`,
    );
...
export const addTransaction = async (
  requestData: Omit<ExpenseSchema, 'RowId' | 'OpeningBalance' | 'ClosingBalance' | 'Timestamp'>,
): Promise<any> => {
  await instance.post(
    `/${getItem(LOCAL_SESSION_KEYS.SHEET_ID)}/values/${TRANSACTION_TAB_NAME}!A:Z:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
    {
      majorDimension: 'ROWS',
...
export const updateTransaction = async (
  requestData: Omit<ExpenseSchema, 'OpeningBalance' | 'ClosingBalance' | 'Timestamp'>,
): Promise<any> => {
...
export const addSheetHeaderContents = async (): Promise<any> => {
  ...
  await addHeaders(`${CATEGORY_SUBCATEGORY_TAB_NAME}!A:D`, [
    CATEGORY_SUBCATEGORY_TAB_HEADERS,
    ...CATEGORY_SUB_CATEGORY_MASTER_DATA,
  ]);
...
const addHeaders = async (sheetNameWithRange: string, rows: string[][]) => {
  await instance.post(
    `/${getItem(LOCAL_SESSION_KEYS.SHEET_ID)}/values/${sheetNameWithRange}:append?valueInputOption=USER_ENTERED&insertDataOption=INSERT_ROWS`,
    {
      majorDimension: 'ROWS',
      values: rows,
    },
